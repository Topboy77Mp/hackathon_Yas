...
export function ImpactPage() {
  ...
  const metrics = [
    { icon: <GroupsOutlined fontSize="small" />, label: "Groupes ouverts", description: "Nombre de groupes d’achat actuellement ouverts à la participation.", value: entier(impact.groups_active) },
    { icon: <TrendingUpOutlined fontSize="small" />, label: "Groupes finalisés", description: "Nombre de groupes ayant atteint leur terme avec des commandes confirmées.", value: entier(impact.groups_successful) },
    { icon: <PeopleAltOutlined fontSize="small" />, label: "Acheteurs inscrits", description: "Nombre de comptes acheteurs enregistrés sur la plateforme.", value: entier(impact.users) },
    { icon: <Inventory2Outlined fontSize="small" />, label: "Unités commandées", description: "Somme des quantités commandées dans les groupes non annulés.", value: entier(impact.units_ordered) },
  ];

  return <Stack spacing={3.5} sx={{ py: 1 }}>
    <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ alignItems: { sm: "flex-start" }, justifyContent: "space-between" }}><Box><Typography color="text.secondary" variant="body2">Impact de la communauté</Typography><Typography variant="h4">Le pouvoir d’achat, en commun.</Typography><Typography color="text.secondary" sx={{ mt: 0.5 }} variant="body2">Des indicateurs calculés à partir de l’activité enregistrée.</Typography></Box><Chip label="Indicateurs cumulés" size="small" variant="outlined" /></Stack>
    ...
    <Card variant="outlined"><Box sx={{ p: 3, pb: 1.5 }}><Typography variant="h5">L’activité en chiffres</Typography><Typography color="text.secondary" sx={{ mt: 0.25 }} variant="body2">Une lecture utile pour suivre la dynamique des achats groupés.</Typography></Box><Divider /><Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", "@media (max-width: 900px)": { gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }, "@media (max-width: 540px)": { gridTemplateColumns: "1fr" } }}>{metrics.map((metric, index) => <Box key={metric.label} sx={{ display: "flex", gap: 1.25, minWidth: 0, p: 2.5, borderRight: { md: index < metrics.length - 1 ? 1 : 0 }, borderBottom: { xs: index < metrics.length - 1 ? 1 : 0, md: 0 }, borderColor: "divider" }}><Box sx={{ color: "text.secondary", pt: 0.25 }}>{metric.icon}</Box><Box><Stack direction="row" sx={{ alignItems: "center" }}><Typography color="text.secondary" variant="caption">{metric.label}</Typography><KpiTooltip label={metric.label} description={metric.description} /></Stack><Typography sx={{ fontWeight: 700, fontVariantNumeric: "tabular-nums" }} variant="h5">{metric.value}</Typography></Box></Box>)}</Box></Card>
