...
export function OverviewPage() {
  ...
  const metrics = [
    { icon: <GroupsOutlined fontSize="small" />, label: "Groupes ouverts", value: entier(dashboard.groups), color: "text.primary" },
    { icon: <ShoppingCartOutlined fontSize="small" />, label: "Commandes reçues", value: entier(dashboard.orders), color: "text.primary" },
    { icon: <Inventory2Outlined fontSize="small" />, label: "Unités réservées", value: entier(dashboard.units), color: "text.primary" },
    { icon: <AccountBalanceWalletOutlined fontSize="small" />, label: "Économies communauté", value: impact.data ? fcfa(impact.data.community_savings) : "…", color: "success.main" },
  ];
  ...
  return <Stack spacing={3.5} sx={{ py: 1 }}>
    ...
    <Card variant="outlined"><Box sx={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", "@media (max-width: 960px)": { gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }, "@media (max-width: 600px)": { gridTemplateColumns: "1fr" } }}>{metrics.map((metric, index) => <Box key={metric.label} sx={{ display: "flex", gap: 1.25, minWidth: 0, p: 2.5, borderRight: { md: index < metrics.length - 1 ? 1 : 0 }, borderBottom: { xs: index < metrics.length - 1 ? 1 : 0, md: 0 }, borderColor: "divider" }}><Box sx={{ color: "text.secondary", pt: 0.25 }}>{metric.icon}</Box><Box sx={{ minWidth: 0 }}><Typography color="text.secondary" variant="caption">{metric.label}</Typography><Typography color={metric.color} noWrap sx={{ fontWeight: 700, fontVariantNumeric: "tabular-nums" }} variant="h6">{metric.value}</Typography></Box></Box>)}</Box></Card>
